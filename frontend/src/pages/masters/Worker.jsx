...
import {
  ...
  X, Save, User, Phone, Briefcase, IndianRupee, ToggleLeft, ToggleRight
} from "lucide-react";
...
const validateForm = (form) => {
  const errors = {};
  if (!form.name.trim()) errors.name = "Worker name is required.";
  if (!form.role.trim()) errors.role = "Role is required.";
  if (!form.salary) {
    errors.salary = "Salary is required.";
  } else if (isNaN(form.salary) || Number(form.salary) <= 0) {
    errors.salary = "Enter a valid salary greater than 0.";
  }
...
const EMPTY_FORM = { name: "", mobile: "", role: "", salary: "" };
...
const Worker = () => {
  ...
  const fetchWorkers = useCallback(async (signal) => {
    ...
    try {
      ...
      if (err.name !== "CanceledError") toast.error("Failed to load workers");
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setPage((prev) => (workers.length === 1 && prev > 1 ? prev - 1 : prev));
      triggerRefresh();
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await axios.put(`/workers/${editingId}`, payload);
  ...
  // Monthly estimate from per-day salary
  const monthlyEstimate = form.salary && !isNaN(form.salary)
    ? (Number(form.salary) * 26).toLocaleString("en-IN")
    : null;
    ...
    <Layout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Worker Master</h2>
            <p className="text-slate-500 text-xs sm:text-sm font-medium">Manage your workforce and salaries</p>
          </div>
          <button
          ...
          >
            ...
            <span className="text-sm">{showForm ? "Close" : "Add Worker"}</span>
          </button>
        </div>
          ...
          <div className="bg-white p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-xl border border-indigo-50 animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                ...
                <InputGroup label="Worker Name" icon={<User size={14} />} error={errors.name}>
                  <input
                    type="text"
                    value={form.name}
                ...
                <InputGroup label="Role" icon={<Briefcase size={14} />} error={errors.role}>
                  <input
                    type="text"
                    value={form.role}
                    onChange={(e) => setForm({ ...form, role: e.target.value })}
                    className={`form-input-custom ${errors.role ? "border-rose-300" : ""}`}
                    placeholder="e.g. Driver"
                  />
                </InputGroup>

                <InputGroup label="Salary / Day" icon={<IndianRupee size={14} />} error={errors.salary}>
                  <input
                    type="number"
                    value={form.salary}
                    onChange={(e) => setForm({ ...form, salary: e.target.value })}
                    className={`form-input-custom ${errors.salary ? "border-rose-300" : ""}`}
                    placeholder="0.00"
                  />
                </InputGroup>
              ...
              {monthlyEstimate && (
                <div className="flex items-center gap-2 px-4 py-3 bg-indigo-50 rounded-xl border border-indigo-100">
                  <span className="text-xs font-bold text-indigo-600">
                    Est. Monthly (×26 days): ₹{monthlyEstimate}
                  </span>
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-slate-50">
                ...
                >
                  <Save size={18} /> {editingId ? "Update Worker" : "Save Worker"}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="relative min-h-[300px]">
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50/80 text-slate-400 text-[10px] uppercase tracking-widest font-black">
                <tr>
                  ...
                  <th className="px-6 py-5 text-center">Salary / Day</th>
                  <th className="px-6 py-5 text-center">Monthly Est.</th>
                ...
                {workers.length === 0 && !loading ? (
                  ...
                  workers.map((w) => (
                    <tr key={w.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-8 py-4 font-bold text-slate-800">{w.name}</td>
                      <td className="px-6 py-4 text-slate-500">{w.mobile || <span className="text-slate-300">—</span>}</td>
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[11px] font-bold">{w.role}</span>
                      </td>
                      <td className="px-6 py-4 text-center font-black text-slate-900">₹{w.salary}</td>
                      <td className="px-6 py-4 text-center text-slate-500 text-xs font-semibold">
                        ₹{(Number(w.salary) * 26).toLocaleString("en-IN")}
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-8 py-4">
                        <div className="flex items-center gap-2 justify-end">
                          ...
                          >
                            {w.status === "active" ? <ToggleRight size={18} /> : <ToggleLeft size={18} />}
                          </button>
                          <button
          ...
          <div className="md:hidden space-y-4">
            {workers.length === 0 && !loading ? (
              ...
              workers.map((w) => (
                <div key={w.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <h4 className="font-bold text-slate-900 text-base leading-tight">{w.name}</h4>
                      <div className="flex gap-2">
                        <span className="text-[10px] font-bold text-slate-400 uppercase">{w.role}</span>
                        {w.mobile && (
                          <>
                            <span className="text-[10px] font-bold text-slate-400">•</span>
                            <span className="text-[10px] font-bold text-slate-400">{w.mobile}</span>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 py-3 border-y border-slate-50">
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[9px] font-bold text-slate-400 uppercase">Per Day</span>
                      <span className="text-sm font-black text-indigo-600">₹{w.salary}</span>
                    </div>
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[9px] font-bold text-slate-400 uppercase">Monthly Est.</span>
