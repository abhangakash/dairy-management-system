...
import {
  ...
  X, Save, Package, Calendar, Minus,
  User, Monitor, ClipboardList
} from "lucide-react";
...
const validateForm = (form) => {
  ...
  if (!form.produced_unit || Number(form.produced_unit) < 1)
    errors.produced_unit = "Produced units must be at least 1.";
  if (!form.date) errors.date = "Date is required.";
  return errors;
};

const EMPTY_FORM = { product_id: "", produced_unit: 1, date: getTodayDate() };
...
const Production = () => {
  ...
  // Load product dropdown once
  useEffect(() => {
    const loadOptions = async () => {
      try {
        const res = await axios.get("/production/options/products");
  ...
  const fetchProductions = useCallback(async (signal) => {
    setLoading(true);
    try {
      ...
      if (err.name !== "CanceledError") toast.error("Failed to load production entries");
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    fetchProductions(controller.signal);
  ...
  const handleUnitChange = (delta) => {
    setForm((prev) => {
      const next = Number(prev.produced_unit) + delta;
      if (next < 1) return prev;
      return { ...prev, produced_unit: next };
    });
  };
  ...
  const handleDelete = async (sr_no) => {
    ...
    try {
      await axios.delete(`/production/${sr_no}`);
      toast.success("Entry deleted successfully!");
      setPage((prev) => (productions.length === 1 && prev > 1 ? prev - 1 : prev));
      triggerRefresh();
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.post("/production", {
        ...
        produced_unit: Number(form.produced_unit),
        date: form.date,
      });
    ...
    <Layout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Production</h2>
            <p className="text-slate-500 text-xs sm:text-sm font-medium">Record daily production entries</p>
          </div>
          <button
          ...
          <div className="bg-white p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-xl border border-indigo-50 animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                ...
                {/* Produced Units — stepper */}
                <InputGroup label="Produced Units" icon={<ClipboardList size={14} />} error={errors.produced_unit}>
                  <div className={`flex items-center bg-slate-50 border rounded-[0.875rem] overflow-hidden transition-all ${
                    errors.produced_unit ? "border-rose-300" : "border-slate-100 focus-within:border-indigo-500 focus-within:bg-white focus-within:shadow-[0_0_0_3px_rgba(99,102,241,0.08)]"
                  }`}>
                    <button
                      type="button"
                      onClick={() => handleUnitChange(-1)}
                      disabled={Number(form.produced_unit) <= 1}
                      className="px-4 py-3 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors font-black text-lg"
                    >
                      <Minus size={16} />
                    </button>
                    <input
                      type="number"
                      value={form.produced_unit}
                      onChange={(e) => {
                        const val = Number(e.target.value);
                        if (val >= 1) setForm({ ...form, produced_unit: val });
                      }}
                      className="flex-1 text-center bg-transparent outline-none font-black text-slate-800 text-base py-3"
                      min={1}
                    />
                    <button
                      type="button"
                      onClick={() => handleUnitChange(1)}
                      className="px-4 py-3 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 transition-colors font-black text-lg"
                    >
                      <Plus size={16} />
                    </button>
                  </div>
        ...
        <div className="relative min-h-[300px]">
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50/80 text-slate-400 text-[10px] uppercase tracking-widest font-black">
                <tr>
                  ...
                  <th className="px-6 py-5 text-center">Entered By</th>
              ...
              <tbody className="divide-y divide-slate-50 text-sm">
                {productions.length === 0 && !loading ? (
                  ...
                  productions.map((p) => (
                    <tr key={p.sr_no} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-8 py-4 text-slate-400 text-xs font-bold">{p.sr_no}</td>
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[11px] font-bold">{p.product_name}</span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className="font-black text-slate-900 text-base">{p.produced_unit}</span>
                      </td>
                      <td className="px-6 py-4 text-center text-slate-500 text-xs font-medium">{formatDate(p.date)}</td>
                      <td className="px-6 py-4 text-center text-slate-400 text-xs">{formatDateTime(p.en_on)}</td>
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <User size={12} className="text-slate-400" />
                          <span className="text-xs font-bold text-slate-600">{p.en_by || "—"}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <Monitor size={12} className="text-slate-400" />
                          <span className="text-[11px] text-slate-400 font-medium">{p.ip || "—"}</span>
                        </div>
                      </td>
                      <td className="px-8 py-4">
                        <div className="flex justify-end">
                          <button
                            onClick={() => handleDelete(p.sr_no)}
          ...
          <div className="md:hidden space-y-4">
            {productions.length === 0 && !loading ? (
              ...
              productions.map((p) => (
                <div key={p.sr_no} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <h4 className="font-bold text-slate-900 text-base leading-tight">{p.product_name}</h4>
                      <span className="text-[10px] font-bold text-slate-400 uppercase">{formatDate(p.date)}</span>
                    </div>
                    <span className="text-[10px] font-bold text-slate-300">#{p.sr_no}</span>
                  </div>

                  <div className="grid grid-cols-2 gap-2 py-3 border-y border-slate-50">
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[9px] font-bold text-slate-400 uppercase">Produced Units</span>
                      <span className="text-xl font-black text-indigo-600">{p.produced_unit}</span>
                    </div>
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[9px] font-bold text-slate-400 uppercase">Entered On</span>
                      <span className="text-[11px] font-bold text-slate-600 text-center">{formatDateTime(p.en_on)}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="flex items-center gap-1.5 bg-slate-50 rounded-xl px-3 py-2">
                      ...
                      <span className="text-[11px] font-bold text-slate-600 truncate">{p.en_by || "—"}</span>
                    </div>
                    <div className="flex items-center gap-1.5 bg-slate-50 rounded-xl px-3 py-2">
                      <Monitor size={12} className="text-slate-400 shrink-0" />
                      <span className="text-[11px] text-slate-400 truncate">{p.ip || "—"}</span>
                    </div>
                  </div>
